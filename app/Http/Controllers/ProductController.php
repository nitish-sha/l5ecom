...
{
 $categories = Category::with('childrens')->get();
 return view(Auth::user()->role->name.'.products.create',compact('product', 'categories'));
}
...
public function update(StoreProduct $request, Product $product)
{
  ...
  if($product->save()){
    $product->categories()->attach($request->category_id, ['created_at'=>now(), 'updated_at'=>now()]);
    return redirect(route(Auth::user()->role->name.'.product.index'))->with('message', "Product Successfully Updated!");
  }else{
