...
<li class="breadcrumb-item "><a href="{{route('seller.product.index')}}">Products</a></li>
<li class="breadcrumb-item active" aria-current="page">Add/Edit Products</li>
@endsection
@section('content')
<h2 class="modal-title">Add/Edit Products</h2>
<form  action="@if(isset($product)) {{route('seller.product.update', $product)}} @else {{route('seller.product.store')}} @endif" method="post" accept-charset="utf-8" enctype="multipart/form-data">
	<div class="row">
		@csrf
		@if(isset($product))
		@method('PUT')
...
$('#btn-add').on('click', function(e){
		...
		$.get("{{route('seller.product.extras')}}").done(function(data){
