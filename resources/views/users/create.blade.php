...
<li class="breadcrumb-item "><a href="{{route('seller.profile.index')}}">users</a></li>
<li class="breadcrumb-item active" aria-current="page">Add/Edit users</li>
@endsection
@section('content')
<h2 class="modal-title">Add/Edit users</h2>
<form  action="@if(isset($user)) {{route('seller.profile.update', $user)}} @else {{route('seller.profile.store')}} @endif" method="post" accept-charset="utf-8" enctype="multipart/form-data">
	<div class="row">
		@csrf
		@if(isset($user))
		@method('PUT')
		...
		<div class="col-lg-9">
			<div class="form-group row">
				...
				<div class="col-sm-12 col-md-6">
					...
					<p class="small">{{route('seller.profile.index')}}/<span id="url">{{$user->profile->slug}}</span>
...
$.ajax({
type: 'GET',
url: "{{route('seller.profile.states')}}/" + id
}).then(function (data) {
...
$.ajax({
type: 'GET',
url: "{{route('seller.profile.cities')}}/" + id
}).then(function (data) {
	// create the option and append to Select2
	for(i=0; i< data.length; i++){
		var item = data[i]
